	...
	<div class="row">
		...
		<div class="col-lg-3">
			<div class="ibox float-e-margins settings-box">
				...
				<?php
				$isAccountTokens = isset($tpl['query']['variant']) && in_array($tpl['query']['variant'], array('account','forgot'));
				$isExtraPaymentsTokens = isset($tpl['query']['variant']) && in_array($tpl['query']['variant'], array('extra_payments'));
				?>
				<div class="ibox-content">
					<div class="row">
						<div class="hbNotifyTokens hbBookingTokens" style="display:<?php echo $isAccountTokens || $isExtraPaymentsTokens ? 'none' : ''; ?>;">
							<?php __('opt_o_email_body_text'); ?>
						</div>
						<div class="hbNotifyTokens hbAccountTokens" style="display:<?php echo $isAccountTokens ? '' : 'none'; ?>;">
							<?php __('opt_o_email_account_tokens'); ?>
						</div>
						<div class="hbNotifyTokens hbExtraPaymentsTokens" style="display:<?php echo $isExtraPaymentsTokens ? '' : 'none'; ?>;">
							<?php __('opt_o_email_extra_payments_tokens'); ?>
						</div>
					</div>
				</div>
			</div>
